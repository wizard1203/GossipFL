...
from .PSGDAggregator import PSGDAggregator
from .PSGDTrainer import PSGDTrainer
from .PSGDClientManager import PSGDClientManager
from .PSGDServerManager import PSGDServerManager
...
def FedML_PSGD_distributed(process_id, worker_number, device, comm, model, train_data_num, train_data_global, test_data_global,
...
def init_server(args, device, comm, rank, size, model, train_data_num, train_data_global, test_data_global,
    ...
    # aggregator
    worker_num = size - 1
    aggregator = PSGDAggregator(train_data_global, test_data_global, train_data_num,
                                train_data_local_dict, test_data_local_dict, train_data_local_num_dict,
                                worker_num, device, args, model_trainer, perf_timer, metrics)
    # start the distributed training
    server_manager = PSGDServerManager(args, aggregator, comm, rank, size, "MPI", perf_timer, metrics)
...
def init_client(args, device, comm, process_id, size, model, train_data_num, train_data_local_num_dict,
    ...
    trainer = PSGDTrainer(client_index, train_data_local_dict, train_data_local_num_dict, test_data_local_dict,
                            train_data_num, device, args, model_trainer, perf_timer, metrics)
    client_manager = PSGDClientManager(args, trainer, comm, process_id, size, "MPI", perf_timer, metrics)
    client_manager.run()
