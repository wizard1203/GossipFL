    ...
    if client_index is None:
        ...
        train_dl = data.DataLoader(data.ConcatDataset(
            StackOverflowDataset(
                os.path.join(data_dir, DEFAULT_TRAIN_FILE), client_index,
                "train", {
                    "input": lambda x: utils.preprocess_input(x, data_dir),
                    "target": lambda y: utils.preprocess_target(y, data_dir)
                }) for client_index in range(DEFAULT_TRAIN_CLIENTS_NUM)),
                                   batch_size=train_bs,
                                   shuffle=True)

        test_dl = data.DataLoader(data.ConcatDataset(
            StackOverflowDataset(
                os.path.join(data_dir, DEFAULT_TEST_FILE), client_index, "test",
                {
                    "input": lambda x: utils.preprocess_input(x, data_dir),
                    "target": lambda y: utils.preprocess_target(y, data_dir)
                }) for client_index in range(DEFAULT_TEST_CLIENTS_NUM)),
                                  batch_size=test_bs,
                                  shuffle=True)
        ...
        train_ds = StackOverflowDataset(
            os.path.join(data_dir, DEFAULT_TRAIN_FILE), client_index, "train", {
                "input": lambda x: utils.preprocess_input(x, data_dir),
                "target": lambda y: utils.preprocess_target(y, data_dir)
            })
        ...
        if client_index >= DEFAULT_TEST_CLIENTS_NUM:
            test_dl = None
        else:
...
def load_partition_data_federated_stackoverflow_lr(
    ...
    if os.path.exists(cache_path):
        ...
        for client_index in tqdm.tqdm(range(DEFAULT_TRAIN_CLIENTS_NUM)):
            train_data_local, test_data_local = get_dataloader(
                dataset, data_dir, batch_size, batch_size, client_index)
            local_data_num = len(train_data_local.dataset)
            data_local_num_dict[client_index] = local_data_num
            # logging.info("client_index = %d, local_sample_number = %d" %
            #              (client_index, local_data_num if test_data_local==None else len(test_data_local.dataset)))
            # logging.info(
            #     "client_index = %d, batch_num_train_local = %d"
            # % (client_index, len(train_data_local)))
