...
def get_dataloader(dataset, data_dir, train_bs, test_bs, client_index = None):
    ...
    if client_index is None:
        ...
        client_id_train = client_ids_train[client_index]
        ...
        if client_index <= len(client_ids_test) - 1:
            client_id_test = client_ids_test[client_index]
...
def load_partition_data_federated_cifar100(dataset, data_dir, batch_size=DEFAULT_BATCH_SIZE):
    ...
    for client_index in range(DEFAULT_TRAIN_CLINETS_NUM):
        ...
        logging.info("client_index = %d, batch_num_train_local = %d" % (client_index, len(train_data_local)))
