...
def fed_noise_load_dataset(args, process_id=0):
    kwargs = {}
    # kwargs["batch_size"] = args.fed_noise_dataset_batch_size // len(args.fed_noise_dataset_list)
    # kwargs["usage"] = "auxiliary"
    # kwargs["image_size"] = args.dataset_load_image_size
    batch_size = args.fed_noise_dataset_batch_size // len(args.fed_noise_dataset_list)
    image_size = args.dataset_load_image_size

    logging.info(f"Loading datasets for fednoise ------  fed_noise_dataset_list :{args.fed_noise_dataset_list}")

    dataset_list = []
    datadir_list = []

    # May we support multiple dataset in the future.
    for i, dataset_name in enumerate(args.fed_noise_dataset_list):
        if dataset_name == "Sub-Tiny-ImageNet-200":
            # assert not args.num_classes > 200
            kwargs["max_classes"] = args.num_classes
            dataset_list.append(dataset_name)
            datadir_list.append(args.fed_noise_datadir_list[i])
    ...
    if args.fed_noise_data == "dataset":
        # if in args.fed_noise_dataset_list:
        train_dl_dict, test_dl_dict, train_ds_dict, test_ds_dict, \
            class_num_dict, train_data_num_dict, test_data_num_dict = load_multiple_centralized_dataset(
                load_as="fed_noise_aux", args=args, process_id=process_id, mode="centralized", task="centralized",
